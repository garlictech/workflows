...
Well, all the build, server, etc. commands run inside the development Docker container you've just built in the previous steps. Your code is mounted inside the container (see scripts in the `docker` folder). Therefore, when you `npm install --save` a new package, it is installed locally only. You have to re-run `npm run build:dev`. This recreates the development image, and incorporates the newly installed package. Do not forget the `--save` flag! The image is build by the content of the `package.json` file.
